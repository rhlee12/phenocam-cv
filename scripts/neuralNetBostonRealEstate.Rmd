...
## An example of a linear model vs a neural network using Boston Real Estate Data
...
```{r bostonData1}
apply(bos.data,2,function(x) sum(is.na(x)))
#There are no missing data, good. We proceed by randomly splitting the data into a train and a test set, 
#then we fit a linear regression model and test it on the test set. Note that I am using the gml() function instead of the lm() 
#this will become useful later when cross validating the linear model.
index <- sample(1:nrow(bos.data),round(0.75*nrow(bos.data)))
train <- bos.data[index,]
test <- bos.data[-index,]
...
pr.nn_ <- pr.nn$net.result*(max(bos.data$medv)-min(bos.data$medv))+min(bos.data$medv)
test.r <- (test_$medv)*(max(bos.data$medv)-min(bos.data$medv))+min(bos.data$medv)
...
Cross validation is another very important step of building predictive models. While there are different kind of cross validation methods, the basic idea is repeating the following process a number of times:
...
library(boot)
set.seed(200)
lm.fit <- glm(medv~.,data=bos.data)
cv.glm(bos.data,lm.fit,K=10)$delta[1]
```
...
pbar$init(k)
for(i in 1:k){
    index <- sample(1:nrow(bos.data),round(0.9*nrow(bos.data)))
    train.cv <- scaled[index,]
    test.cv <- scaled[-index,]
    nn <- neuralnet(f,data=train.cv,hidden=c(5,2),linear.output=T)   
    pr.nn <- compute(nn,test.cv[,1:13])
    pr.nn <- pr.nn$net.result*(max(bos.data$medv)-min(bos.data$medv))+min(bos.data$medv)   
    test.cv.r <- (test.cv$medv)*(max(bos.data$medv)-min(bos.data$medv))+min(bos.data$medv)   
